...
export const createCheckoutSession = async (req, res) => {
    try {
        ...
        const restaurent = await Restaurent.findById(checkoutSessionRequest.restaurentId).populate("menus");
        if (!restaurent) {
        ...
        const totalAmount = checkoutSessionRequest.cartItems.reduce((total, item) => total + item.price * item.quantity, 0);
        ...
        const session = await stripe.checkout.sessions.create({
            ...
            metadata: {
                ...
                images: JSON.stringify(menuItems.map((menuItem) => menuItem.image)),
            },
        });
        if (!session.url) {
            return res.status(404).json({
