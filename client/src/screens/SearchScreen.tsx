...
function SearchScreen() {
  ...
  const { searchRestaurent, appliedFilter, searchItems, setAppliedFilter } =
    useRestaurentStore();
  useEffect(() => {
    searchRestaurent(searchText!, searchQuery, appliedFilter);
        ...
        <div className="my-2 flex-1">
          <div className="flex gap-2">
            <Input
              ...
              placeholder="Search by city , food ...."
            />
            <Button
              onClick={() =>
                searchRestaurent(searchText!, searchQuery, appliedFilter)
          ...
          <div className="font-bold text-2xl my-3">
            <h1>({searchItems?.data?.length})Search result found</h1>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-4 mt-4 md:flex-row">
            {searchItems?.data?.map((restaurent: any) => (
              <Card className="bg-white shadow-lg" key={restaurent?._id}>
                ...
                <CardContent className=" p-4">
                  ...
                  <div className="flex gap-2 items-center ">
                    <MapPin size={16} />
                    <p>
                      City : <span>{restaurent?.city}</span>
                    </p>
                  </div>
                  <div className="flex gap-2 items-center ">
                    <Globe size={16} />
                    <p>
                      Country : <span>{restaurent?.country}</span>
                    </p>
                  </div>
                  <div className="flex gap-3  text-white my-2">
                    {restaurent?.cuisines.map(
